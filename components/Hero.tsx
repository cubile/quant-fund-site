export function Hero() {
  return (
    <section className="space-y-6 sm:space-y-8">
      <p className="text-xs font-semibold uppercase tracking-[0.32em] text-accent">
        Systematic Quantitative Investment Research
      </p>

      <h1 className="max-w-3xl text-3xl font-semibold tracking-tight text-white sm:text-5xl md:text-6xl">
        CUBILE PROP FUND
      </h1>

      <p className="max-w-2xl text-sm leading-relaxed text-[#9ca3af] sm:text-base">
        Institutional derivatives portfolios built on quantitative research and
        disciplined risk frameworks. The program focuses on robust portfolio
        construction, regime-aware risk budgets, and consistent implementation
        rather than discretionary trading.
      </p>
    </section>
  );
}